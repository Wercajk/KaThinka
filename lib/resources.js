...
/**
 * Build middleware for OPTIONS method
 * @param {Array} methods
...
res.buildDefaultRoutes = function() {
  ...
  this.app.koa.use(route.options('/', this.defineOptions(['GET'])));
...
res.buildUserResources = function() {
  ...
  while (i--) {
    ...
    this.app.koa.use(route.options('/' + name, this.defineOptions(['POST', 'GET'])));

    // List resource options
    this.app.koa.use(route.options('/' + name + '/:label', this.defineOptions(['GET', 'PATCH', 'PUT', 'DELETE'])));
