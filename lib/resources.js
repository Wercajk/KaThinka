...
function Resources(app) {
  ...
  // Root of API resource files
  // TODO: Should be array and set in config
 ...
 * Load App files
 *
 * Check resource folder and load resource files
 *
 * @return {Void}
 */
res.loadFiles = function() {
    // Check if resource folder existst
...
res.buildDefaultRoutes = function() {
  ...
  this.app.koa.use(route.get('/', function *() {
    ...
    if ( dbResponse.length > 0 ) {
      ...
      this.throw('Could\'t reach database backend.', 500);
